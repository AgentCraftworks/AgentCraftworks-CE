...
export function detectFailurePatterns(logContent: string): FailurePattern[] {
  return FAILURE_PATTERNS.filter((fp) => fp.pattern.test(logContent));
}

export interface CoachRunInfo {
  name?: string | null | undefined;
  run_number: number;
  html_url: string;
}

export const COACH_COMMENT_MARKER = "CI Coach Analysis";
export const COACH_MAX_COMMENT_AGE_MINUTES = 5;

export function buildCoachComment(
  run: CoachRunInfo,
...
/**
 * Pure selection logic behind `findRecentCoachComment`: picks the most recent
 * bot comment carrying the CI Coach marker if it is fresh enough.
 */
export function selectRecentCoachComment(
  ...
  const coachComments = comments
    .filter(
      (c) =>
        ...
        c.body?.includes(COACH_COMMENT_MARKER),
    )
    .sort(
      (a, b) =>
  ...
  if (minutesOld > COACH_MAX_COMMENT_AGE_MINUTES) {
    return null;
  }
...
async function findRecentCoachComment(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      return selectRecentCoachComment(comments);
    } catch (err) {
      lastError = err;
...
export async function main(): Promise<void> {
  ...
  // Analyze failures
  const detectedPatterns = detectFailurePatterns(logContent);

  const body = buildCoachComment(run, detectedPatterns, parseInt(runId, 10));
...
if (isDirectRun(import.meta.url)) runJob("CI Coach", main);
