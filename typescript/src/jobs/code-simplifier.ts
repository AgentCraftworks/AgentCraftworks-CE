 ...
 *
 * Engagement Level: T3 (Collaborator) — creates draft PRs with code changes.
 */
...
export const SIMPLIFICATION_RULES: SimplificationRule[] = [
  {
  ...
  },
  {
    ...
    testAfter: "const { name: name, age: age } = user;",
  },
  {
...
export function applySimplifications(content: string, filePath: string): { newContent: string; simplifications: Simplification[] } {
...
export function buildSimplifierPrBody(
  fileChanges: Pick<FileChange, "path" | "simplifications">[],
): { title: string; body: string; total: number } {
  ...
  const body = [
    ...
    "",
    "---",
    "*Generated by GH-AW Code Simplifier — Engagement Level: T3 (Collaborator)*",
  ].join("\n");

  return {
    title: `🧹 Code Simplifier: ${totalSimplifications} modernizations`,
    body,
    total: totalSimplifications,
  };
}
...
export async function main(): Promise<void> {
  ...
  const { title, body: prBody, total: totalSimplifications } = buildSimplifierPrBody(fileChanges);
...
if (isDirectRun(import.meta.url)) runJob("Code Simplifier", main);
