...
export function extractLogSnippet(logs: string, pattern: RegExp, contextLines = 3): string {
...
export function diagnoseFailures(logs: string): DiagnosticResult[] {
...
export type Severity = FailureCategory["severity"];

export function overallSeverity(diagnostics: DiagnosticResult[]): Severity {
...
export function collectLabels(diagnostics: DiagnosticResult[]): string[] {
...
export interface DoctorRunInfo {
...
export function buildDiagnosticReport(
  ...
  return {
    title: `🏥 CI Failure: ${run.name} #${run.run_number} — ${severity.toUpperCase()}`,
    body: lines.join("\n"),
  };
}
...
export async function main(): Promise<void> {
  ...
  const severity = overallSeverity(diagnostics);
  const allLabels = collectLabels(diagnostics);
  const { title, body } = buildDiagnosticReport(run, failedJobs, diagnostics, severity);
...
if (isDirectRun(import.meta.url)) runJob("CI Doctor", main);
