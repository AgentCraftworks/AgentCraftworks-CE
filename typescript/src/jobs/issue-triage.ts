...
/** Jaccard similarity over whitespace-separated lowercase tokens. */
...
export function analyzeIssue(title: string, body: string): Omit<TriageResult, "similarIssues"> {
...
): TriageResult["similarIssues"] {
  return candidates
    .filter((i) => i.number !== issueNum)
    .map((i) => ({
      number: i.number,
      title: i.title,
      similarity: calculateSimilarity(title, i.title),
    }))
...
export function buildTriageComment(result: TriageResult): string {
  const lines: string[] = [
...
export async function main(): Promise<void> {
  ...
  const similarIssues = findSimilarIssues(issue.title, issueNum, recentIssues);
  ...
  const body = buildTriageComment(result);
...
if (isDirectRun(import.meta.url)) runJob("Issue Triage", main);
