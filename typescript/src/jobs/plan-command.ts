 ...
 *
 * Engagement Level: T3 (Collaborator) — creates sub-issues with structured content.
 */
...
export const PLAN_PATTERNS: PlanPattern[] = [
  {
    type: "api",
...
export const DEFAULT_PLAN: SubTaskTemplate[] = [
  {
...
/** Returns the first pattern with at least two keyword hits, or null. */
export function detectPattern(content: string): PlanPattern | null {
...
export function effortToEmoji(effort: string): string {
  switch (effort) {
    case "small":
      return "🟢";
    case "medium":
...
export function buildSubIssueTitle(
  parentTitle: string,
  index: number,
  total: number,
  task: SubTaskTemplate,
): string {
  return `[${parentTitle}] ${index + 1}/${total}: ${task.title}`;
}

export function buildSubIssueBody(parentNumber: number, parentTitle: string, task: SubTaskTemplate): string {
  return [
    ...
    "",
    "---",
    "*Generated by GH-AW Plan Command — Engagement Level: T3 (Collaborator)*",
  ].join("\n");
}

export function buildPlanSummary(
  createdIssues: Array<{ number: number; title: string }>,
  patternType: string,
): string {
  return [
...
export async function main(): Promise<void> {
  ...
  for (let i = 0; i < subTasks.length; i++) {
    ...
    const subTitle = buildSubIssueTitle(parentIssue.title, i, subTasks.length, task);
    const subBody = buildSubIssueBody(issueNum, parentIssue.title, task);
  ...
  await octokit.issues.createComment({
    ...
    repo,
    issue_number: issueNum,
    body: buildPlanSummary(createdIssues, patternType),
  });
...
if (isDirectRun(import.meta.url)) runJob("Plan Command", main);
